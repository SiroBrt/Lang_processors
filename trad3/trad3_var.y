...
function_rec: { sprintf (func_prefix, "main_") ; }
              MAIN '(' ')' '{' code '}'
              { sprintf(temp, "(defun main () %s)\n", $5.code) ; 
                $$.code = gen_code(temp) ; }
            | IDENTIF { sprintf (func_prefix, "func_") ; }
              '(' arg_def ')' '{' code '}' 
              { sprintf(temp, "(defun %s (%s) %s)\n", $1.code, $3.code, $6.code) ; 
                $$.code = gen_code(temp) ; }
                function_rec
              { sprintf(temp, "%s%s", temp, $8.code) ; }
...
control_sentence:
            ...
            | FOR '(' axiom_sentence ';' expression ';' IDENTIF '=' expression ')' '{' code '}' 
              { sprintf (temp, "%s (loop while %s do %s (setq %s %s))", $3.code , $5.code, $12.code, $7.code, $9.code) ;
                $$.code = gen_code (temp) ; }
            ;
