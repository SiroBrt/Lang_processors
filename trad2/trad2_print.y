...
axiom:        axiom_sentence ';'             { printf ("%s\n", $1.code) ; }
                r_axiom                      { ; }
            | MAIN '(' ')' '{' code '}'      { sprintf (temp, "(defun main %c()%c%s)", 31, 31, $5.code) ;
                                               my_print(temp) ; }
            ;
...
axiom_sentence:
              INTEGER rec_def                { $$ = $2 ; }
            | IDENTIF '=' operand            { sprintf (temp, "(setq %s %s)", $1.code, $3.code) ;
                                               $$.code = gen_code(temp) ; }
            ;
            
sentence:     INTEGER rec_def                { $$ = $2 ; }
            | IDENTIF '=' expression         { sprintf (temp, "(setq %s %c%s)%c", $1.code, 31, $3.code, 31) ;
                                               $$.code = gen_code(temp) ; }
            | PUTS '(' STRING ')'            { sprintf (temp, "(print %c\"%s\")%c", 31, $4.code, 31) ;  
                                               $$.code = gen_code (temp) ; }
...
control_sentence:
              ...
              { sprintf (temp, "(loop while %c%s%c do %s)", 31, $3.code, 31, $6.code) ;  
                $$.code = gen_code (temp) ; }
            ;

rec_print:    expression ',' rec_print       { sprintf (temp, "(princ %c%s)%c %s", 31, $1.code, 31, $3.code);
                                               $$.code = gen_code (temp) ; }
            | STRING ',' rec_print           { sprintf (temp, "(princ %c\"%s\")%c %s", 31, $1.code, 31, $3.code);
                                               $$.code = gen_code (temp) ; }
            | expression                     { sprintf (temp, "(princ %c%s)%c", 31, $1.code, 31);
                                               $$.code = gen_code(temp) ; }
            | STRING                         { sprintf (temp, "(princ %c\"%s\")%c", 31, $1.code, 31);
                                               $$.code = gen_code(temp) ; }
            ;

rec_def:      definition ',' rec_def         { sprintf (temp, "%s\n%s",$1.code, $3.code);
                                               $$.code = gen_code (temp) ; }
            | definition                     { $$ = $1 ; }
            ;
